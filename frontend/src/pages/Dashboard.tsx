...
export function Dashboard() {
  ...
  return (
    <div className="max-w-4xl mx-auto pt-10 px-4 relative pb-20">
      <button
        onClick={handleLogout}
        className="absolute top-4 right-4 bg-slate-800/80 hover:bg-red-500/80 text-slate-300 hover:text-white px-5 py-2 rounded-lg backdrop-blur-sm border border-white/10 transition-all duration-300 text-sm font-medium shadow-lg"
      >
        Sair da Conta
      </button>

      <div className="bg-slate-800/60 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl p-8 mt-10">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            {(['todas', 'pendentes', 'concluidas'] as FilterType[]).map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 md:flex-none px-4 py-2 rounded-md text-sm font-medium transition-colors capitalize ${filter === f
        ...
        <div className="space-y-4">
          {filteredTasks.length === 0 ? (
            ...
            filteredTasks.map(task => (
              <div
                key={task.id}
                className={`flex justify-between items-center p-4 border rounded-lg transition-all duration-300 group ${task.status === 'concluida'
              ...
              >
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors flex-shrink-0 ${task.status === 'concluida'
                  ...
                  <div className="flex-1 mr-4">
                    {editingTaskId === task.id ? (
                      ...
                      <h3
                        ...
                        className={`font-medium text-lg transition-colors cursor-text ${task.status === 'concluida' ? 'line-through text-slate-500' : 'text-slate-100'
                          }`}
                      >
                        {task.title}
                      </h3>
